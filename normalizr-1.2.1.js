...
(function normalizr() {
    ...
    */
    ...
    var __GLOBALTHIS__ = (function() {
        ...
        function getThis() {
            if (isWindow()) { return window } // if we're here it's almost certainly Internet Explorer so check for a window.
            else {
     ...
     */
    var __CONTEXT__ = (function() {
        if (isWindow()) { return 'window' } // all browsers
    ...
    var funcRegex = /^(\[object (Function|GeneratorFunction)\])$/;
    ...
    // ToInteger | ECMA-262/11.0 | Unary operator throws TypeError on BigInt, and Symbol primitives as per the spec. Number() constructor does not.
    ...
    function ToUint32(x) { return x >>> 0 } // ToUint32 | Almost never used, as inlining is faster

    function ToNumber(n) { return +n } // ToNumber
    ...
    // ToSoftNumber | Converts any value to a Number type (may return NaN) via number operator or unary operator without throwing a TypeError
    ...
    //---------------------------------- `is` functions ----------------------------------------//
     ...
     // If this much effort was taken to simulate a browser environment, assume they know what they're doing
    function isWindow() {
        if (typeof window === 'object' && window && (window.window === window) && typeof window.document === 'object') {return true} return false;
    }

     // Determines that a value is explicitly `NaN`, and not a value coerced to `NaN`
    function isActualNaN(val) { return val !== val }
    ...
    ;(function(GLOBAL, CONTEXT, factory) {
        ...
        function __factory__( GLOBAL, context, undefined ) { // FACTORY: the factory modular code. BEGINNING OF FACTORY FUNCTION
        ...
        function isAccessorObject(obj) { 
            if (typeof obj === 'object' && obj) { return (('get' in obj) || ('set' in obj)) } return false;
        }
         ...
         *  Object.defineProperty has optional keys, which default to: {enumerable: false, writable: false, configurable: false, value: undefined}
        */
        ...
        function define(obj, property, value, options, overwrite) {
            ...
            var AccessorsObject, ConfigObject = (function(O) {
                // Only defines whether enumerable, writable, or configurable. Defaults to {}
                ...
                if (typeof options === 'object' && options) {
                    O[e] = !!getOwnProp(options, e);
                    O[c] = !!getOwnProp(options, c);
                    O[w] = !!getOwnProp(options, w);
                }
                return O;
            })( {} );
            ...
            if (arguments.length === 2) {
                ConfigObject[e] = ConfigObject[c] = ConfigObject[w] = true; // CraftedObject.value: undefined
            } 

            if (!isAccessorObject(value)) {
                ConfigObject.value = value; // standard object descriptor. use as-is.
            } else {

                /* If `isAccessorObject` is `true`, `value` is an object with `get` / `set` accessors, which we will assume to be intentional.
                    Example: define(window, 'getter', { get() { return 2; }, set(a) { window.current = a; }, }, { enumerable: true, configurable: true, writable: false }); 
                    ...
                    objects are passed by reference so we will use new object instead of trying to make a deep copy.
                    if the object has accessors, we're using the value itself as the object, and we're overwriting the defaults
                */

                if (('value' in value) || ('writable' in value) || ('writable' in ConfigObject)) {
                    throw new TypeError("Invalid property descriptor. Cannot both specify accessors and a value or writable attribute");
                }

                AccessorsObject = value;
                AccessorsObject[e] = !!ConfigObject[e]; // coerce to boolean as the property may not be initialized
                AccessorsObject[c] = !!ConfigObject[c];
             // AccessorsObject.enumerable = (hasOwn(value, 'enumerable')) ? value.enumerable : !!ConfigObject.enumerable;

            } 

            // Options object overwrites internal object.
            ...
            if (overwrite || !(property in obj)) { // only if property isn't defined, or we want to overwrite
                try {
                    ...
                    Object.defineProperty(obj, property, (AccessorsObject || ConfigObject));
